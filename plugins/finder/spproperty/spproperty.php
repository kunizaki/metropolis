<?php
/**
 * @package     SP Property Plugins
 * @subpackage  Finder.Spproperty
 *
 * @copyright   Copyright (C) 2020 - 2024 JoomShaper. All rights reserved.
...
if(!class_exists('SppropertyHelper'))
{
	require_once JPATH_ROOT . '/components/com_spproperty/helpers/helper.php';
}

/**
 * Plugin class for Page Builder smart search finder plugin
 */
class PlgFinderSpproperty extends Adapter
{
	/**
	 * The plugin context.
	 *
	 ...
	 * @since	1.0.0
	 */
	protected $extension = 'com_spproperty';
	 ...
	 */
	protected $table = '#__spproperty_properties';
	...
	{
		if ($context === 'com_spproperty.property')
		{
			$id = $table->id;
		}
	...
	protected function index( Result $item, $format = 'html')
	{
		...
		$item->context = 'com_spproperty.property';
		...
		$link = 'index.php?option=com_spproperty&view=property&id=' . $item->id;
	...
	{
		JLoader::register('SppropertyRouter', JPATH_SITE . '/components/com_spproperty/route.php');
	...
	protected function getListQuery($query = null)
	{
		...
		$query = $query instanceof DatabaseQuery ? $query : $db->getQuery(true)
			->select('a.id, a.title AS title, a.description AS body, a.features_text, a.created AS start_date')
			->select('a.created_by, a.modified, a.modified_by, a.language')
			->select('a.access, a.published AS state, a.ordering')

			->select('u.name')
			->from('#__spproperty_properties AS a')
	...
	public static function getActiveMenu($pageId) {
		...
		$query->where($db->quoteName('link') . ' LIKE '. $db->quote('%option=com_spproperty&view=property&id='. $pageId .'%'));
